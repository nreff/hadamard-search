...
- a dedicated `lp333-multiplier` analyzer now separates unconditional multiplier equivalence from conditional stabilizer assumptions
  ...
  - a bounded `hadamard analyze lp333-multiplier --invariant-shift31` graph diagnostic now profiles the first actual mixed shift `(3,1)` for the two surviving order-`3` targets: it keeps the same `1,296` exact row-pair survivors, finds only `42` fixed-row triple targets and `6` nonfixed row-sum targets, and now prints the min-fill order and bag scale for the exact graph solver shape; the fixed-row graph has `13` variables with domain `8`, min-fill width `5`, and max bag domain states `262,144`, while the nonfixed row-orbit graph has `37` Boolean variables, `105` edges, weighted edge count `108`, max degree `6`, min-fill width `10` / `9`, and max bag domain states `2,048` / `1,024` for column generators `10` / `26`
...
3. Continue the two surviving non-column-trivial order-`3` `row_units={1,4,7}` multiplier targets by deriving mixed CRT constraints for the `col_units={1,10,26}` row-orbit action, starting from the `1,296` fixed-row-compatible row marginals; corrected single-shift marginals and the first relaxed two-shift coupling do not prune, and the `(3,1)` graph profile now points to an actual invariant sign-table lift using min-fill bag DP rather than another raw frontier/value materialization.
