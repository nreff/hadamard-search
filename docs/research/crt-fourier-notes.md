...
That result moves the multiplier line past the pure row-marginal stage and kills the simplest implementation of the row action. The fixed-row check narrows the non-column-trivial branch, the corrected `(3,0)` and `(0,1)` single-shift marginals are too weak to narrow it further, and the actual row-stabilizer lift rejects the order-`6` mixed targets. The next exact sieve has to couple mixed shifts or lift actual invariant sign tables for the two surviving order-`3` nonfixed row orbits, because row-sum constraints alone no longer distinguish the remaining non-column-trivial subgroups.
