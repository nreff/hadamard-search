...
- the fixed-row side of shift `(3,1)` is modest (`13` variables, domain `8`, `32` edges, weighted edge count `35`, self terms `2`, max degree `6`, min-fill width `5`) and has max bag domain states `262,144`
- the nonfixed row-orbit side has `37` Boolean variables, `105` edges, weighted edge count `108`, self terms `3`, max degree `6`, and min-fill width `9` or `10` depending on the column generator, but its max bag domain states are only `1,024` or `2,048`
...
That result moves the multiplier line past the pure row-marginal stage and kills the simplest implementation of the row action. The fixed-row check narrows the non-column-trivial branch, the corrected `(3,0)` and `(0,1)` single-shift marginals are too weak to narrow it further, the first relaxed two-shift coupling is still too weak, exact fixed-row coupling still lacks a practical state representation, and the actual row-stabilizer lift rejects the order-`6` mixed targets. The shift `(3,1)` graph diagnostic now gives a more constructive warning: the remaining target sets are small and naive value-DP is the wrong representation, but the min-fill bag-domain bounds are small enough that a graph-structured exact invariant sign-table lift is the next concrete sieve to try.
