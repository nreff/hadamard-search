...
- a previous fast `2^13` result for `(0,1)` is no longer treated as recorded, because it enumerated rows fixed by column multiplication by `10`; the nonfixed order-`3` row orbits require arbitrary base rows whose copies are column-multiplied
- the arbitrary-row table has been restored; the corrected exact run takes about `176s` and leaves all `1,296` fixed-row-compatible row-marginal pairs alive, so `(0,1)` does not prune at this marginal level
- a relaxed coupled `(3,0)+(0,1)` check now exists behind `--col10-coupled`; it couples the arbitrary nonfixed row-orbit base rows through a joint table, still relaxes the fixed-row coupling, and also leaves all `1,296` fixed-row-compatible marginals alive
- exact fixed-row coupling was attempted both as a naive three-fixed-row frontier and as a shared fixed-triple target table narrowed to the exact fixed-row-compatible survivors; neither produced a full stats line in the practical debug window, so another raw fixed-row frontier is unlikely to be the right next discriminator
- the multiplier analyzer also has an opt-in CRT component orbit view behind `--crt-components`, which prints the coordinate orbits as `(mod 9, mod 37)` residue pairs and a focused `row_units={1,4,7}` hypothesis summary; that family currently has `15` surviving hypotheses split across subgroup orders `3,6,9,12,18,36`, with order-`3` column units `[1]` / `[1,10,26]` and order-`6` column units `[1,36]` / `[1,10,11,26,27,36]`; the nontrivial mixed-CRT target set is now the four hypotheses in `crt_component_mixed_crt_targets`, with order split `3:2,6:2`, and the summary now includes `pairs`, `mass`, `pair_samples`, and `col_orbits`, but those four targets currently tie on `pairs=12`, `mass=119903105952`, `row_orbits=0|1,4,7|2,5,8|3|6`, `allowed_bundles=1064`, and the same top pair samples; `col_orbits` only separates the order-`3` and order-`6` buckets, and the remaining distinction is the explicit `subgroup_crt` label
- the CRT component view now also computes actual invariant-row-stabilizer lifts for those mixed targets: the two order-`3` targets have `113` invariant table orbits (`1:3,3:110`) and reproduce the `1,296` exact row-pair survivors with aggregate pattern mass about `10^60.818`; the two order-`6` targets have `59` invariant table orbits (`1:3,3:2,6:54`) but their stronger row stabilizer leaves `0` row-pair candidates after the active bundle-pair join
- the first actual mixed shift `(3,1)` structure check now exists behind `--invariant-shift31`; it is deliberately a bounded graph/target diagnostic, not a full exact feasibility enumerator
- for the two surviving order-`3` targets, that diagnostic still sees `1,296` exact row-pair survivors, with only `42` fixed-row triple targets and `6` nonfixed row-sum targets
- the fixed-row side of shift `(3,1)` is modest (`13` variables, domain `8`, `32` edges, weighted edge count `35`, self terms `2`, max degree `6`, min-fill width `5`) and now has an exact linear-frontier order of width `6` (`8^6 = 262,144` states)
- the nonfixed row-orbit side has `37` Boolean variables, `105` edges, weighted edge count `108`, self terms `3`, max degree `6`, and min-fill width `9` or `10` depending on the column generator; the current frontier-order heuristic gives width `11` for both column generators (`2^11 = 2,048` states)
- an opt-in exact value-table scaffold for this one mixed shift now exists behind `--invariant-shift31-exact`; in release mode it completes in about `189s`, with fixed targets `42 / 42`, nonfixed targets `6 / 6`, max row energy values `84`, and all `1,296` row pairs still feasible
- a bounded value-profile mode now exists behind `--invariant-shift31-profile`; with exact fixed-row suffix target reachability and the new frontier order, the raw profile still caps at `100,000` values, but the active-width pressure is much lower than before
- a compressed profile mode now exists behind `--invariant-shift31-compressed-profile`; with a `100,000` row-signature cap, the nonfixed side completes exactly (`31,020` / `26,598` signatures for column generators `10` / `26`), and in release mode a `30,000,000` cap completes the fixed side exactly with peak `20,849,958` row signatures and `58,360,095` energy bits
...
That result moves the multiplier line past the pure row-marginal stage and kills the simplest implementation of the row action. The fixed-row check narrows the non-column-trivial branch, the corrected `(3,0)`, `(0,1)`, and exact `(3,1)` single-shift marginals are too weak to narrow it further, the first relaxed two-shift coupling is still too weak, and the actual row-stabilizer lift rejects the order-`6` mixed targets. The next concrete sieve should couple multiple mixed shifts or add a stronger invariant across the fixed and nonfixed row orbits.
